<template>
  <div class="wrap">
    <div class="container container-big">
      <div class="top">
        <div class="left">
          <h4 class="title">{{ categoryId.name }}</h4>
          <div class="buttons">
            <NuxtLink
              :to="`/${$route.params.lang}/products/${categoryId.id}/?category=${categoryId.id}`"
              class="butn"
              :class="{ active: categoryId.id == $route.query.category }"
              >{{ $store.state.translations['main.all'] }}</NuxtLink
            >
            <NuxtLink
              v-for="item in categoryId.children"
              :key="item.id"
              :to="`/${$route.params.lang}/products/${categoryId.id}/?category=${item.id}`"
              class="butn"
              :class="{ active: item.id == $route.query.category }"
            >
              {{ item.name }}
            </NuxtLink>
          </div>
          <p class="info">
            <!-- {{ $store.state.translations['main.all'] }} {{ products.length }} -->
          </p>
        </div>
        <div class="right">
          <div class="form">
...
export default {
  ...
  watch: {
    async sortProducts(val) {
      this.categoryProducts = await this.sortCategories()
    },
  ...
  methods: {
    ...
    async searchAny(value) {
      const seekItems = await searchApi.getSearch({
        category: this.$route.query.category,
        search: value,
      })

      this.products = seekItems
    },
  },
}
</script>
